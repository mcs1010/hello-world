...
public class MentalCommandWithCloudProfile implements Runnable {
		...
		while (true) {
			...
			if (state == EdkErrorCode.EDK_OK.ToInt()) {
				...
				if (eventType == Edk.IEE_Event_t.IEE_UserAdded.ToInt())
					...
					System.out.println(Edk.INSTANCE.IEE_EmoEngineEventGetEmoState(eEvent, eState) + "UPDATED ");
				}
	...
	private static void SavingLoadingFunction(int userCloudID, int engineUserID, boolean save, String profileName) {
		...
		if (save) {
			...
			if (profileID.getValue() >= 0) {
				...
				if (EmotivCloudClient.INSTANCE.EC_UpdateUserProfile(userCloudID, engineUserID,profileID.getValue()) == EdkErrorCode.EDK_OK.ToInt()) {
			...
			if (getNumberProfile > 0) {
				if (EmotivCloudClient.INSTANCE.EC_LoadUserProfile(userCloudID, engineUserID, profileID.getValue(),-1) == EdkErrorCode.EDK_OK.ToInt())
	...
	private static void handleMentalCommandEvent(IntByReference engineUserID, Pointer cognitiveEvent) {
		...
		case 2: {
			...
			Edk.INSTANCE.IEE_MentalCommandSetTrainingControl(engineUserID.getValue(),Edk.IEE_MentalCommandTrainingControl_t.MC_ACCEPT.getType());
			break;
		}
	...
	public static boolean handleUserInput(IntByReference userId, IntByReference userCloudId, String profileName,String line) {
		...
		case "3": {
			...
			Edk.INSTANCE.IEE_MentalCommandSetTrainingAction(userId.getValue(),EmoState.IEE_MentalCommandAction_t.MC_RIGHT.ToInt());
			Edk.INSTANCE.IEE_MentalCommandSetTrainingControl(userId.getValue(),Edk.IEE_MentalCommandTrainingControl_t.MC_START.getType());
			break;
		}
		case "4": {
			System.out.println("Starting training LEFT!");
			Edk.INSTANCE.IEE_MentalCommandSetTrainingAction(userId.getValue(),EmoState.IEE_MentalCommandAction_t.MC_LEFT.ToInt());
			Edk.INSTANCE.IEE_MentalCommandSetTrainingControl(userId.getValue(),Edk.IEE_MentalCommandTrainingControl_t.MC_START.getType());
			break;
		}
		case "5": {
